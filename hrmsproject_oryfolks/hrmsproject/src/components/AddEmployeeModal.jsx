...
export default function AddEmployeeModal({ open, onClose, onEmployeeCreated }) {
    ...
    // ─── Auto-generate Employee ID and corporate email from backend ───────────
    const generateIdAndEmail = useCallback(async (firstName, lastName, role) => {
        if (!firstName.trim() || !lastName.trim() || !role) return;

        setGeneratingId(true);
        try {
            const res = await api(
                `/api/employees/generate-id?role=${encodeURIComponent(role)}&firstName=${encodeURIComponent(firstName.trim())}&lastName=${encodeURIComponent(lastName.trim())}`
            );
            if (res.ok) {
                ...
                setFormData(prev => ({
                    ...prev,
                    oryfolksId: payload.employeeId || prev.oryfolksId,
                    corporateEmail: payload.emailPrefix || prev.corporateEmail,
                }));
            }
        } catch (e) {
    ...
    // Trigger generation whenever firstName, lastName, or role changes (on step 2)
    useEffect(() => {
        if (formData.firstName && formData.lastName && formData.role) {
            generateIdAndEmail(formData.firstName, formData.lastName, formData.role);
        }
    }, [formData.firstName, formData.lastName, formData.role, generateIdAndEmail]);
    ...
    const handleRoleChange = (role) => {
        setFormData(prev => ({ ...prev, role, oryfolksId: "", corporateEmail: "" }));
        // generateIdAndEmail will fire via useEffect
    };
    ...
    const handleSubmit = async () => {
        ...
        const jdVal = validateJoiningDate(formData.joiningDate);
        if (!jdVal.isValid) {
            newErrors.joiningDate = jdVal.error;
        }
    ...
    const handleCreateUser = async () => {
        ...
        try {
            const employeeData = {
                ...formData,
                // Save digits only, exactly as entered (no country-code prefix).
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className={`w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-sm font-medium text-brand-blue focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue transition-all outline-none ${fieldErrors.firstName ? 'border-red-500 bg-red-50' : ''}`}
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className={`w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-sm font-medium text-brand-blue focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue transition-all outline-none ${fieldErrors.middleName ? 'border-red-500 bg-red-50' : ''}`}
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className={`w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-sm font-medium text-brand-blue focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue transition-all outline-none ${fieldErrors.lastName ? 'border-red-500 bg-red-50' : ''}`}
                                    ...
                                    <div className="md:col-span-3">
                                        ...
                                        <input
                                            ...
                                            className={`w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-sm font-medium text-brand-blue focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue transition-all outline-none ${fieldErrors.email ? 'border-red-500 bg-red-50' : ''}`}
                                    ...
                                    <div>
                                        ...
                                        <div className="flex">
                                            <select
                                                ...
                                                className="bg-gray-50 border border-gray-200 rounded-l-lg px-2 py-2.5 text-sm font-medium text-brand-blue focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue transition-all outline-none border-r-0"
                                            >
                                            ...
                                            <input
                                                ...
                                                className={`flex-1 w-full bg-gray-50 border border-gray-200 rounded-r-lg px-4 py-2.5 text-sm font-medium text-brand-blue focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue transition-all outline-none ${fieldErrors.phoneNumber ? 'border-red-500 bg-red-50' : ''}`}
                                                placeholder={phoneCountryCode === '+81' ? '11 digit number' : '10 digit number'}
                                                maxLength={getMobileMaxDigits(phoneCountryCode)}
                                            />
                                        </div>
                                        <FormFieldError error={fieldErrors.phoneNumber} show={!!fieldErrors.phoneNumber} />
                                        <p className="text-[10px] text-gray-400 mt-1">India: 10 digits | Japan: 11 digits</p>
                                    </div>
                                    <div>
                                        ...
                                        <DateInput
                                            ...
                                            className={`w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-sm font-medium text-brand-blue focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue transition-all outline-none ${fieldErrors.dateOfBirth ? 'border-red-500 bg-red-50' : ''}`}
                                        />
                                    ...
                                    <div>
                                        ...
                                        <select
                                            ...
                                            className={`w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-sm font-medium text-brand-blue focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue transition-all outline-none ${fieldErrors.gender ? 'border-red-500 bg-red-50' : ''}`}
                                        >
                            ...
                            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    ...
                                    <div className="md:col-span-2">
                                        ...
                                        <select
                                            ...
                                            className={`w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-sm font-medium text-brand-blue focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue transition-all outline-none ${fieldErrors.role ? 'border-red-500 bg-red-50' : ''}`}
                                        >
                                    ...
                                    {/* Company ID — auto-generated, read-only indication */}
                                    <div>
                                        <label className="block text-xs font-bold text-gray-600 mb-2 flex items-center gap-1">
                                            Company ID *
                                            {generatingId && (
                                                <svg className="animate-spin w-3 h-3 text-brand-blue ml-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" opacity="0.25"></circle><path d="M4 12a8 8 0 018-8" opacity="0.75"></path></svg>
                                            )}
                                            {!generatingId && formData.oryfolksId && (
                                                <span className="ml-1 text-emerald-600 text-[10px] font-bold">✓ Auto-generated</span>
                                            )}
                                        </label>
                                        <input
                                            type="text"
                                            ...
                                            readOnly
                                            className="w-full bg-gray-100 border border-gray-200 rounded-lg px-4 py-2.5 text-sm font-bold text-brand-blue outline-none cursor-not-allowed"
                                            placeholder={formData.role ? "Generating…" : "Select a role first"}
                                        />
                                        {formData.role && formData.firstName && formData.lastName && (
                                            <p className="text-[10px] text-gray-400 mt-1">
                                                Format: OF-{formData.role === "HR" ? "HR" : "IT"}-{formData.lastName.substring(0, 1).toUpperCase()}{formData.firstName.substring(0, 1).toUpperCase()}-0001
                                            </p>
                                        )}
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className={`w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-sm font-medium text-brand-blue focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue transition-all outline-none ${fieldErrors.designation ? 'border-red-500 bg-red-50' : ''}`}
                                            placeholder="Software Engineer"
                                        />
                                        <FormFieldError error={fieldErrors.designation} show={!!fieldErrors.designation} />
                                    </div>

                                    {/* Corporate Email — auto-generated but editable */}
                                    <div>
                                        <label className="block text-xs font-bold text-gray-600 mb-2 flex items-center gap-1">
                                            Corporate Email *
                                            {generatingId && (
                                            ...
                                            {!generatingId && formData.corporateEmail && (
                                                <span className="ml-1 text-emerald-600 text-[10px] font-bold">✓ Auto-generated (editable)</span>
                                            )}
                                        </label>
                                        <div className="flex">
                                            <input
                                                ...
                                                onChange={(e) => setFormData({ ...formData, corporateEmail: e.target.value.toLowerCase().replace(/[^a-z0-9._-]/g, "") })}
                                                ...
                                                placeholder={formData.role ? (generatingId ? "Generating…" : "e.g. g.shalini") : "Select a role first"}
                                            />
                                        ...
                                        {formData.lastName && formData.firstName && (
                                            <p className="text-[10px] text-gray-400 mt-1">
                                                Auto-filled as <strong>{formData.lastName.substring(0, 1).toLowerCase()}.{sanitize(formData.firstName)}</strong> — you may edit if needed.
                                            </p>
                                        )}
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <DateInput
                                            ...
                                            onChange={(e) => {
                                                ...
                                                setFieldErrors({ ...fieldErrors, joiningDate: validation.error });
                                            }}
                                            className={`w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-sm font-medium text-brand-blue focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue transition-all outline-none ${fieldErrors.joiningDate ? 'border-red-500 bg-red-50' : ''}`}
                                        />
                                ...
                                {formData.role && (
                                    <div className="mt-4 p-3 bg-brand-blue/5 border border-brand-blue/20 rounded-lg flex items-start gap-2">
                                        <svg className="w-4 h-4 text-brand-blue mt-0.5 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>
                                        <p className="text-xs text-brand-blue/80 font-medium">
                                            Company ID and Corporate Email are <strong>automatically generated</strong> based on the employee's name and role. They are unique and cannot be manually changed.
                                        </p>
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="absolute inset-0 z-50 flex items-center justify-center bg-brand-blue/90 backdrop-blur-sm p-4">
                        <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
                            ...
                            <div className="p-6 space-y-5">
                                ...
                                <div className="bg-brand-blue/5 border border-brand-blue/20 rounded-lg p-3.5 flex items-start gap-2">
                                    <svg className="w-4 h-4 text-brand-blue mt-0.5 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>
                                    <p className="text-xs text-brand-blue/80 font-medium">
                                        An onboarding welcome email will be automatically sent to the employee with their credentials and a secure password setup link.
                                    </p>
                                </div>
