...
const splitPhone = (phone) => {
  ...
  // Legacy values stored with an explicit known country-code prefix
  const match = s.match(/^(\+81|\+91)(\d+)$/);
  if (match) {
    return { countryCode: match[1], number: match[2] };
  }
  // Digits-only stored value: infer country from length (11 digits → Japan, else India)
  const digits = s.replace(/\D/g, "");
  return { countryCode: digits.length === 11 ? "+81" : "+91", number: digits };
};

// Year-only picker: shows the selected year as YYYY with a calendar icon that opens a
// grid of selectable years. Styled to match the other Education Details input boxes.
function YearPicker({ value, onChange, disabled, minYear, maxYear, hasError }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className={`w-full flex items-center justify-between bg-white border-none rounded-lg px-3 py-2 text-sm font-bold text-brand-blue text-left ${hasError ? 'ring-2 ring-red-500 bg-red-50' : ''} ${disabled ? 'cursor-not-allowed opacity-80' : 'cursor-pointer'}`}
      >
        <span className={value ? '' : 'text-brand-blue/30 font-medium'}>{value || 'YYYY'}</span>
        <svg className="w-4 h-4 text-brand-blue/40 ml-2 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {open && !disabled && (
        <div className="absolute z-20 mt-1 w-full max-h-48 overflow-y-auto bg-white rounded-lg shadow-lg border border-gray-200 p-2 grid grid-cols-3 gap-1">
          {years.map((y) => (
            <button
              key={y}
              type="button"
              onClick={() => { onChange(String(y)); setOpen(false); }}
              className={`px-2 py-1.5 rounded-md text-sm font-bold transition-all ${String(y) === String(value) ? 'bg-brand-blue text-white' : 'text-brand-blue hover:bg-brand-blue/10'}`}
            >
              {y}
            </button>
          ))}
...
export default function EmployeeOwnProfile({ hideSidebar = false }) {
  ...
  const fetchEmpData = async () => {
    ...
    try {
      ...
      const populatedForm = {
        ...
        currentAddress: data.presentAddress && data.presentAddress !== 'To be updated' ? data.presentAddress : "",
  ...
  const convertMonthToDate = (monthValue) => {
    if (!monthValue || typeof monthValue !== 'string' || !monthValue.trim()) return null;
  ...
  const handleSaveProfile = async (skipValidation = false) => {
    ...
    try {
      ...
      // Diagnostic: confirm Current Address (sent as presentAddress) is in the request body.
      console.log("[Profile Save] currentAddress =", JSON.stringify(form.currentAddress), "-> presentAddress =", JSON.stringify(payload.presentAddress));
      console.log("[Profile Save] full payload:", payload);
      ...
      // Notify the surrounding dashboard so the header ribbon (name/designation/photo)
      // refreshes immediately from the updated session — no page refresh needed.
      window.dispatchEvent(new Event("userPhotoUpdated"));
  ...
  const photoSrc = form.photoUrl || employee?.photoPath || "";
            ...
            <aside className="lg:col-span-1">
              <div className="bg-[#F8F7F4] rounded-2xl border border-black/5 p-5 lg:sticky lg:top-4">
                <div className="flex flex-col items-center text-center pb-5 border-b border-black/5">
                  ...
                  <h2 className="mt-3 text-[18px] font-medium text-brand-blue leading-tight">{employee?.firstName} {employee?.lastName}</h2>
                  <p className="text-[13px] text-brand-blue/50 mt-0.5">{form.role || employee?.designation || "Ory Folks Team"}</p>
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all text-[13px] whitespace-nowrap border-l-[3px] ${active
                          ...
                          : 'border-transparent text-brand-blue/40 font-medium hover:bg-white/60 hover:text-brand-blue'}`}
                      >
                        <span className={`shrink-0 ${active ? 'text-brand-blue' : 'text-brand-blue/40'}`}>{s.icon}</span>
                        {s.label}
                      </button>
                    );
                  })}
            ...
            <div className="lg:col-span-3">
              <div className="bg-white rounded-2xl shadow-sm border border-black/5 overflow-hidden min-h-[600px]">
                <div className="flex flex-wrap items-center justify-between gap-3 px-6 lg:px-10 py-5 border-b border-gray-100">
                  <h2 className="text-base font-semibold text-brand-blue">{activeSectionLabel}</h2>
                  <div className="flex items-center gap-2">
                    {/* Edit/Save is hidden on the read-only Leave Balance tab; Back stays. */}
                    {activeSection === 'leave_balance' ? null : !editing ? (
                      <button
                        onClick={handleStartEdit}
                        disabled={loading}
                        className="px-5 py-2 bg-brand-blue text-white text-sm font-semibold rounded-lg shadow-sm hover:bg-brand-blue-hover transition-all disabled:opacity-60"
                      >
                        Edit Profile
                      </button>
                    ) : (
                      <button
                        ...
                        disabled={loading}
                        className="px-5 py-2 bg-green-500 text-white text-sm font-semibold rounded-lg shadow-sm hover:bg-green-600 transition-all disabled:opacity-60"
                      >
                      ...
                      <div className="space-y-8">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
                          <div className="sm:col-span-2 space-y-2">
                            <label className="text-[11px] font-bold text-brand-blue/50 uppercase tracking-[0.06em] ml-1">Full Name</label>
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                              <div>
                                <input
                                  ...
                                  disabled={!editing}
                                  maxLength="32"
                                  className={`w-full bg-[#F8F7F4] border-none rounded-xl px-4 py-3 text-sm font-bold text-brand-blue focus:ring-2 focus:ring-brand-yellow/50 transition-all ${fieldErrors.firstName && touched.firstName ? 'ring-2 ring-red-500 bg-red-50' : ''} ${!editing ? 'cursor-not-allowed opacity-80' : ''}`}
                              ...
                              <div>
                                <input
                                  ...
                                  disabled={!editing}
                                  maxLength="32"
                                  className={`w-full bg-[#F8F7F4] border-none rounded-xl px-4 py-3 text-sm font-bold text-brand-blue focus:ring-2 focus:ring-brand-yellow/50 transition-all ${fieldErrors.middleName && touched.middleName ? 'ring-2 ring-red-500 bg-red-50' : ''} ${!editing ? 'cursor-not-allowed opacity-80' : ''}`}
                              ...
                              <div>
                                <input
                                  ...
                                  disabled={!editing}
                                  maxLength="32"
                                  className={`w-full bg-[#F8F7F4] border-none rounded-xl px-4 py-3 text-sm font-bold text-brand-blue focus:ring-2 focus:ring-brand-yellow/50 transition-all ${fieldErrors.lastName && touched.lastName ? 'ring-2 ring-red-500 bg-red-50' : ''} ${!editing ? 'cursor-not-allowed opacity-80' : ''}`}
                          ...
                          {[
                            ...
                            return (
                              <div key={field.name} className="space-y-2">
                                <label className="text-[11px] font-bold text-brand-blue/50 uppercase tracking-[0.06em] ml-1">{field.label}</label>
                                {isDropdown ? (
                                  <select
                                    name={field.name}
                                  ...
                                  <div>
                                    <div className="flex gap-1">
                                      <select
                                        ...
                                        onChange={handleChange}
                                        disabled={isDisabled}
                                        className={`w-28 bg-[#F8F7F4] border-none rounded-xl px-2 py-3 text-sm font-bold text-brand-blue focus:ring-2 focus:ring-brand-yellow/50 transition-all ${isDisabled ? 'cursor-not-allowed opacity-80' : ''}`}
                                      >
                          ...
                          <div className="sm:col-span-2 space-y-2">
                            <label className="text-[11px] font-bold text-brand-blue/50 uppercase tracking-[0.06em] ml-1">Current Address</label>
                            <textarea
                              name="currentAddress"
                              ...
                              className={`w-full bg-[#F8F7F4] border-none rounded-xl px-4 py-3 text-sm font-bold text-brand-blue focus:ring-2 focus:ring-brand-yellow/50 transition-all ${fieldErrors.currentAddress && touched.currentAddress ? 'ring-2 ring-red-500 bg-red-50' : ''} ${!editing ? 'cursor-not-allowed opacity-80' : ''}`}
                            />
                          ...
                          <div className="sm:col-span-2 space-y-2">
                            <label className="text-[11px] font-bold text-brand-blue/50 uppercase tracking-[0.06em] ml-1">Permanent Address</label>
                            <textarea
                              ...
                              rows="2"
                              maxLength="252"
                              className={`w-full bg-[#F8F7F4] border-none rounded-xl px-4 py-3 text-sm font-bold text-brand-blue focus:ring-2 focus:ring-brand-yellow/50 transition-all ${fieldErrors.permanentAddress && touched.permanentAddress ? 'ring-2 ring-red-500 bg-red-50' : ''} ${!editing ? 'cursor-not-allowed opacity-80' : ''}`}
                            />
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            {field.name === 'emergencyPhone' ? (
                              <div className="flex gap-1">
                                <select
                                  ...
                                  onChange={handleChange}
                                  disabled={!editing}
                                  className={`w-28 bg-[#F8F7F4] border-none rounded-xl px-2 py-3 text-sm font-bold text-brand-blue focus:ring-2 focus:ring-brand-yellow/50 transition-all ${(!editing) ? 'cursor-not-allowed opacity-80' : ''}`}
                                >
                                  <option value="+91">+91 (IN)</option>
                                  <option value="+81">+81 (JP)</option>
                                </select>
                                <input
                                  ...
                                  onBlur={handleBlur}
                                  disabled={!editing}
                                  className={`flex-1 bg-[#F8F7F4] border-none rounded-xl px-4 py-3 text-sm font-bold text-brand-blue focus:ring-2 focus:ring-brand-yellow/50 transition-all ${fieldErrors[field.name] ? 'ring-2 ring-red-500' : ''} ${(!editing) ? 'cursor-not-allowed opacity-80' : ''}`}
                                />
                              </div>
                            ) : field.type === 'select' ? (
                              <div>
                                <select
                                  ...
                                  onBlur={handleBlur}
                                  disabled={!editing}
                                  className={`w-full bg-[#F8F7F4] border-none rounded-xl px-4 py-3 text-sm font-bold text-brand-blue focus:ring-2 focus:ring-brand-yellow/50 transition-all ${fieldErrors[field.name] ? 'ring-2 ring-red-500 bg-red-50' : ''} ${!editing ? 'cursor-not-allowed opacity-80' : ''}`}
                                >
                        ...
                        <div className="sm:col-span-2 space-y-2">
                          ...
                          <textarea
                            ...
                            rows="3"
                            maxLength="252"
                            className={`w-full bg-[#F8F7F4] border-none rounded-xl px-4 py-3 text-sm font-bold text-brand-blue focus:ring-2 focus:ring-brand-yellow/50 transition-all ${fieldErrors.emergencyAddress ? 'ring-2 ring-red-500 bg-red-50' : ''} ${!editing ? 'cursor-not-allowed opacity-80' : ''}`}
                          />
                            ...
                            <div key={idx} className="bg-gray-50 p-6 rounded-2xl relative border border-brand-blue/5">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="space-y-1">
                                  ...
                                  <YearPicker value={edu.startYear || ''} onChange={(year) => handleEducationChange(idx, 'startYear', year)} disabled={!editing} minYear={1950} maxYear={new Date().getFullYear()} hasError={!!fieldErrors[`edu_startYear_${idx}`]} />
                                ...
                                <div className="space-y-1">
                                  ...
                                  <YearPicker value={edu.endYear || ''} onChange={(year) => handleEducationChange(idx, 'endYear', year)} disabled={!editing} minYear={1950} maxYear={new Date().getFullYear() + 5} hasError={!!fieldErrors[`edu_endYear_${idx}`]} />
