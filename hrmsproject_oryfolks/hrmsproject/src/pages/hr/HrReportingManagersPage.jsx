  ...
  return (
    <div className="flex h-screen w-screen bg-[#e3edf9] flex-col md:flex-row overflow-hidden font-brand text-brand-blue">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 p-4 md:p-8 space-y-6 overflow-y-auto md:overflow-hidden flex flex-col min-h-0">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 flex-1 min-h-0">
            ...
            <div className="lg:col-span-1 flex flex-col space-y-4 min-h-0">
              ...
              <div className="bg-white rounded-3xl shadow-xl shadow-brand-blue/5 border border-brand-blue/5 overflow-hidden flex-1 flex flex-col min-h-0">
                    ...
                    managers.map((m) => {
                      ...
                      <div
                        ...
                        className={`group relative cursor-pointer p-4 rounded-2xl transition-all border-2 flex items-center gap-4 ${selected && selected.id === m.id
                          ...
                          : mDisabled
                            ? 'bg-[#F1EFE8] border-transparent hover:border-brand-blue/10 text-brand-blue'
                            : 'bg-white border-transparent hover:border-brand-blue/10 hover:bg-gray-50 text-brand-blue'
                          }`}
                      >
                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-lg shadow-sm transition-all ${selected && selected.id === m.id ? 'bg-white text-brand-blue' : mDisabled ? 'bg-[#D3D1C7] text-[#5F5E5A]' : 'bg-brand-blue/5 text-brand-blue/30 group-hover:bg-brand-blue group-hover:text-white'
                          }`}>
                          {(m.fullName || 'U').slice(0, 1)}
                        </div>
                        <div className="flex-1 overflow-hidden">
                          <div className="flex items-center gap-2">
                            <div className={`font-bold text-sm truncate ${mDisabled && !(selected && selected.id === m.id) ? 'text-brand-blue/40' : ''}`}>{m.fullName}</div>
            ...
            <div className="lg:col-span-2 flex flex-col space-y-4 min-h-0">
              ...
              <div className="bg-white rounded-[32px] shadow-2xl shadow-brand-blue/5 border border-brand-blue/5 overflow-hidden flex-1 flex flex-col relative min-h-0">
                {!selected ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {selected.team.filter(emp => emp.id !== selected.id).map(emp => {
                            ...
                            <div key={emp.id} className={`group p-5 border rounded-[20px] flex items-center gap-4 hover:shadow-xl hover:shadow-brand-blue/5 transition-all hover:-translate-y-0.5 card-hover ${empDisabled ? 'bg-[#F1EFE8] border-brand-blue/5' : 'bg-white border-brand-blue/5'}`}>
                              <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-sm font-black transition-colors ${empDisabled ? 'bg-[#D3D1C7] text-[#5F5E5A]' : 'bg-bg-slate text-brand-blue/30 group-hover:bg-brand-blue group-hover:text-white'}`}>
                                {(emp.name || 'U').slice(0, 1)}
                              </div>
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                  <div className={`font-bold truncate text-sm ${empDisabled ? 'text-brand-blue/40' : 'text-brand-blue'}`}>{emp.name}</div>
