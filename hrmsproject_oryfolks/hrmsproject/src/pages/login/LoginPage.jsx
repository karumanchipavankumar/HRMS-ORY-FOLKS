...
const LoginPage = ({ setUser }) => {
  ...
  // the idle timer (sessionStorage flag) or a redirect carrying ?reason=session_expired.
  // Also proactively clear the session when landing on the login page.
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-[460px] p-10 bg-white/70 backdrop-blur-xl rounded-[32px] shadow-2xl relative z-10 border border-white/40 ring-1 ring-black/5 mx-4">
        ...
        {sessionMsg && !error && (
          <div className="bg-amber-50 text-amber-700 p-3 rounded-lg text-center mb-4 text-[13px] font-medium border border-amber-200">
            {sessionMsg}
          </div>
        )}
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          <div className="space-y-1">
            ...
            <input
              type="text"
              value={username}
              onChange={(e) => { setUsername(e.target.value); if (sessionMsg) setSessionMsg(''); }}
              required
          ...
          <div className="space-y-1">
            ...
            <input
              type="password"
              value={password}
              onChange={(e) => { setPassword(e.target.value); if (sessionMsg) setSessionMsg(''); }}
              required
              className="w-full p-3 rounded-lg border border-brand-blue/20 bg-white/50 focus:bg-white focus:border-brand-yellow focus:ring-2 focus:ring-brand-yellow/20 outline-none transition-all"
